...
corpus = SP_df['Line']
# Remove NA values
keep = pd.notna(corpus)
data = corpus[keep]
# Convert the lines to lowercase words w/o punctuation
translator = str.maketrans('', '', string.punctuation)
l = [line.lower().translate(translator).split(' ') for line in data]
vocabulary = [item for sublist in l for item in sublist]
...
def build_dataset(words, n_words):
  ...
  # Assign a key to each unique word
  dictionary = dict()
  for word, _ in count:
...
def plot_with_labels(low_dim_embs, labels, filename):
  ...
  plt.figure(figsize=(8, 7))  # in inches
